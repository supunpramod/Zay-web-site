...
app.post("/api/products", upload.single("img"), async (req, res) => {
  try {
    ...
    const product = new Product({
      ...
      category,
      rating: rating || 0,
      img: `/uploads/${req.file.filename}`, // This will be accessible via the static route
    });
...
app.get("/api/products", async (req, res) => {
  try {
    ...
    const productsWithFullUrls = products.map(product => ({
      ...product.toObject(),
      img: product.img // This should already be the correct path
    }));
  ...
  try {
    ...
    if (req.file) {
      ...
      if (oldProduct && oldProduct.img) {
        const oldImagePath = path.join(__dirname, oldProduct.img);
